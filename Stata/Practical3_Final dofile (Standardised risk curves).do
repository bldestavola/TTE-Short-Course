...
summarize p_risk if time == 4 & intervention==0
summarize p_risk if time == 4 & intervention==1

*we only need the mean values that are stored in one individual (all individuals have these mean values, i.e. risk at each time point)
...
sort id intervention correct time

separate p_risk, by(intervention) 


twoway (line p_risk0 correct_time, sort) (line p_risk1 correct_time, sort) if intervention > -1, ylabel(0(5)40) xlabel(0(1)5) title("Standardised Risk Curves") ytitle("Risk (%)") xtitle("Years of follow-up") /// 
       legend(label(1 "A=0") label(2 "A=1")) note("Risk A0 = 35.7% (29.7% , 38.4%)" "Risk A1 = 33.7% (30.0% , 34.8%)" "Risk difference (r(A1)-r(A0)=-1.9% (-6.5% , 3.0%)", ring(0) pos(1))
